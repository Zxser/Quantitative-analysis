...
class Company:
    ...
    def initialize_traders(self):
        ...
        for i_trader in range(self.num_traders):
            self.traders[i_trader] = Trader(self.num_stock, self.num_factors_max, self.delay_time_max,
        ...
        for i_stock in range(self.num_stock):
            ...
            for idx in reversed(range(self.time_window)):
                self.traders[i_trader].stack_factors(
                    y_train[:, t-self.delay_time_max-1-idx:t-idx], i_stock)
    ...
    def generate_trader_without_singular(self, i_trader, i_stock, t, vbgmm_num_factor, vbgmm_factor_params):
        y_train = self.df_y_train.T.values
        for i_stock in range(self.num_stock):
            list_params = self.sample_params(
                vbgmm_num_factor, vbgmm_factor_params)
                ...
                if bad_traders[i_stock][i_trader]:
                    if self.how_recruit == "gmm":
                        self.generate_trader_without_singular(
                            i_trader, i_stock, t, list_vbgmm[i_stock][0], list_vbgmm[i_stock][1])
                    else:
        ...
        for i_factor in range(num_factor):
            ...
            for key, value in zip(["delay_P", "delay_Q", "stock_P", "stock_Q", "activation_func", "binary_operator"], factor_params):
                if key in ["delay_P", "delay_Q"]:
                    ...
                    dict_params[key] = self.check_param(
                        value, 0, len(self.activation_funcs)-1)
                elif key == "binary_operator":
                    dict_params[key] = self.check_param(
                        value, 0, len(self.binary_operators)-1)
    ...
    def fit_vbgmm(self, i_stock, good_traders):
        ...
        df_factor_params = pd.DataFrame(columns=[
                                        "delay_P", "delay_Q", "stock_P", "stock_Q", "activation_func", "binary_operator"])
        for i_trader, trader in enumerate(self.traders):
            if good_traders[i_stock][i_trader]:
                ...
                df_num_factor = df_num_factor.append(
                    dict_num_factor, ignore_index=True)
        ...
        vbgmm_num_factor = self.VBGMM(
            df_num_factor, n_components=self.num_stock)
        ...
        for i_trader in range(self.num_traders):
            for i_stock in range(self.num_stock):
                if not bad_traders[i_stock][i_trader]:  # Q-パーセンタイル以上の成績の良いTraderのみを対象
                    ...
                    predictions[i_stock][i_trader] = self.traders[i_trader].predict()[
                        i_stock]

        predictions_weighted = np.zeros(self.num_stock)
        for i_stock in range(self.num_stock):
            predictions_weighted[i_stock] = (
                weights[i_stock]*predictions[i_stock]).sum() / (weights[i_stock].sum())
